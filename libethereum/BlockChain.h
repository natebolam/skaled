...
public:
    ...
    std::tuple< ImportRoute, bool, unsigned > sync(
        BlockQueue& _bq, const skale::State& _state, unsigned _max );
    ...
    std::pair< ImportResult, ImportRoute > attemptImport(
        bytes const& _block, const skale::State& _state, bool _mutBeNew = true ) noexcept;

    /// Import block into disk-backed DB.
    /// @returns the block hashes of any blocks that came into/went out of the canonical block
    /// chain.
    ImportRoute import( bytes const& _block, const skale::State& _state, bool _mustBeNew = true );
    ImportRoute import(
        VerifiedBlockRef const& _block, const skale::State& _state, bool _mustBeNew = true );
