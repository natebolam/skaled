...
json_spirit::mValue BlockchainTestSuite::doTests(
    ...
    for ( auto const& i : _input.get_obj() ) {
        ...
        BOOST_REQUIRE_MESSAGE( inputTest.count( "genesisBlockHeader" ),
            "\"genesisBlockHeader\" field is not found. filename: " +
        ...
        if ( _fillin ) {
            ...
            for ( auto& network : allnetworks ) {
                ...
                if ( !Options::get().singleTestNet.empty() &&
                     Options::get().singleTestNet != test::netIdToString( network ) )
                    continue;
            ...
            BOOST_REQUIRE_MESSAGE( inputTest.count( "network" ),
                "\"network\" field is not found. filename: " +
                    TestOutputHelper::get().testFile().string() +
                    " testname: " + TestOutputHelper::get().testName() );
            dev::test::TestBlockChain::s_sealEngineNetwork =
                stringToNetId( inputTest.at( "network" ).get_str() );
...
json_spirit::mValue TransitionTestsSuite::doTests(
    ...
    for ( auto& i : output.get_obj() ) {
        ...
        BOOST_REQUIRE_MESSAGE(
            o.count( "genesisBlockHeader" ), "genesisBlockHeader not found " + testname );
...
void testBCTest( json_spirit::mObject const& _o ) {
    ...
    TestBlock testGenesisBlock( _o.at( "genesisBlockHeader" ).get_obj(), _o.at( "pre" ).get_obj() );
    TestBlockChain testChain( testGenesisBlock );
    assert( testChain.getInterface().isKnown( genesisBlock.blockHeader().hash( WithSeal ) ) );
    ...
    for ( auto const& bl : _o.at( "blocks" ).get_array() ) {
        ...
        if ( blObj["uncleHeaders"].type() != json_spirit::null_type ) {
            BOOST_REQUIRE_MESSAGE( blObj["uncleHeaders"].get_array().size() <= 2,
                "Too many uncle headers in block! " + TestOutputHelper::get().testName() );
        ...
        try {
            ...
            cerr << testName + "Error importing block from fields to blockchain: "
                 << diagnostic_information( _e );
            break;
        }
        ...
        bytes importedblock =
            testChain.getInterface().block( blockFromFields.blockHeader().hash( WithSeal ) );
        ...
        if ( blockFromFields.blockHeader().parentHash() == preHash ) {
            ...
            cnote << "Skipping the balance validation of potential correct block: "
                  << TestOutputHelper::get().testName();
        }

        cnote << "Tested topblock number" << blockNumber << "for chain " << blockChainName
              << testName;
    ...
    BOOST_CHECK_MESSAGE( lastTrueBlockHash == _o.at( "lastblockhash" ).get_str(),
        testName + "Boost check: lastblockhash does not match " + lastTrueBlockHash +
...
// TestFunction
void overwriteBlockHeaderForTest(
    mObject const& _blObj, TestBlock& _block, ChainBranch const& _chainBranch ) {
    ...
    if ( ho.size() != 14 ) {
        ...
        Ethash::setMixHash( tmp,
            ho.count( "mixHash" ) ? h256( ho["mixHash"].get_str() ) : Ethash::mixHash( header ) );
        Ethash::setNonce( tmp,
            ho.count( "nonce" ) ? eth::Nonce( ho["nonce"].get_str() ) : Ethash::nonce( header ) );
        tmp.noteDirty();
    } else
    ...
    if ( ho.count( "populateFromBlock" ) ) {
        ...
        if ( number < importedBlocks.size() ) {
            ...
            cerr << TestOutputHelper::get().testName() +
                        "Could not populate blockHeader from block: there are no block with number!"
                 << TestOutputHelper::get().testName() << "\n";
        }
    }
...
void overwriteUncleHeaderForTest( mObject& uncleHeaderObj, TestBlock& uncle,
    std::vector< TestBlock > const& uncles, ChainBranch const& _chainBranch ) {
    ...
    if ( uncleHeaderObj.count( "sameAsBlock" ) ) {
        ...
        if ( number < importedBlocks.size() )
            ...
            cerr << TestOutputHelper::get().testName() +
                        "Could not create uncle sameAsBlock: there are no block with number "
                 << number;
        return;
    }
    ...
    if ( uncleHeaderObj.count( "sameAsPreviousBlockUncle" ) ) {
        ...
        if ( number < importedBlocks.size() ) {
            ...
            if ( prevBlockUncles.size() )
                ...
                cerr << TestOutputHelper::get().testName() +
                            "Could not create uncle sameAsPreviousBlockUncle: previous block has "
                            "no uncles!"
                     << TestOutputHelper::get().testName() << "\n";
        } else
            cerr << TestOutputHelper::get().testName() +
                        "Could not create uncle sameAsPreviousBlockUncle: there are no block "
                        "imported!"
                 << TestOutputHelper::get().testName() << "\n";
        return;
    }
    ...
    if ( uncleHeaderObj.count( "populateFromBlock" ) ) {
        ...
        if ( number < importedBlocks.size() ) {
            ...
            cerr << TestOutputHelper::get().testName() +
                        "Could not create uncle populateFromBlock: there are no block with number "
                 << number << TestOutputHelper::get().testName() << "\n";
    } else {
    ...
    if ( overwrite != "false" ) {
        uncleHeader = constructHeader( overwrite == "parentHash" ?
            ...
            overwrite == "gasUsed" ? toInt( uncleHeaderObj.at( "gasUsed" ) ) :
                                     uncleHeader.gasUsed(),
            overwrite == "timestamp" ? toInt( uncleHeaderObj.at( "timestamp" ) ) :
...
void compareBlocks( TestBlock const& _a, TestBlock const& _b ) {
    if ( sha3( RLP( _a.bytes() )[0].data() ) != sha3( RLP( _b.bytes() )[0].data() ) ) {
        ...
        cnote << toHexPrefixed( RLP( _a.bytes() )[0].data() ) << "vs"
              << toHexPrefixed( RLP( _b.bytes() )[0].data() );
    }
...
void checkExpectedException( mObject& _blObj, Exception const& _e ) {
    ...
    BOOST_REQUIRE_MESSAGE( isNetException || isAllNetException,
        TestOutputHelper::get().testName() + " block import thrown unexpected Excpetion! (" +
            exWhat + ")" );
    ...
    BOOST_REQUIRE_MESSAGE( exWhat.find( exExpect ) != string::npos,
        TestOutputHelper::get().testName() +
            " block import expected another exeption: " + exExpect + ", but got: " + exWhat );
}
...
void eraseJsonSectionForInvalidBlock( mObject& _blObj ) {
    // if exception is thrown, RLP is invalid and no blockHeader, Transaction list, or Uncle list
    // should be given
...
void checkBlocks(
    ...
    BOOST_CHECK_MESSAGE(
        blockHeaderFromFields.hash( WithoutSeal ) == blockFromRlp.hash( WithoutSeal ),
        _testname + "hash in given RLP not matching the block hash!" );
    BOOST_CHECK_MESSAGE( blockHeaderFromFields.parentHash() == blockFromRlp.parentHash(),
        _testname + "parentHash in given RLP not matching the block parentHash!" );
    BOOST_CHECK_MESSAGE( blockHeaderFromFields.sha3Uncles() == blockFromRlp.sha3Uncles(),
        _testname + "sha3Uncles in given RLP not matching the block sha3Uncles!" );
    BOOST_CHECK_MESSAGE( blockHeaderFromFields.author() == blockFromRlp.author(),
        _testname + "author in given RLP not matching the block author!" );
    BOOST_CHECK_MESSAGE( blockHeaderFromFields.stateRoot() == blockFromRlp.stateRoot(),
        _testname + "stateRoot in given RLP not matching the block stateRoot!" );
    BOOST_CHECK_MESSAGE(
        blockHeaderFromFields.transactionsRoot() == blockFromRlp.transactionsRoot(),
        _testname + "transactionsRoot in given RLP not matching the block transactionsRoot!" );
    BOOST_CHECK_MESSAGE( blockHeaderFromFields.receiptsRoot() == blockFromRlp.receiptsRoot(),
    ...
    BOOST_CHECK_MESSAGE( blockHeaderFromFields.gasLimit() == blockFromRlp.gasLimit(),
        "testname + gasLimit in given RLP not matching the block gasLimit!" );
    BOOST_CHECK_MESSAGE( blockHeaderFromFields.gasUsed() == blockFromRlp.gasUsed(),
        _testname + "gasUsed in given RLP not matching the block gasUsed!" );
    BOOST_CHECK_MESSAGE( blockHeaderFromFields.timestamp() == blockFromRlp.timestamp(),
        _testname + "timestamp in given RLP not matching the block timestamp!" );
    BOOST_CHECK_MESSAGE( blockHeaderFromFields.extraData() == blockFromRlp.extraData(),
        _testname + "extraData in given RLP not matching the block extraData!" );
    // BOOST_CHECK_MESSAGE(Ethash::mixHash(blockHeaderFromFields) == Ethash::mixHash(blockFromRlp),
    // _testname + "mixHash in given RLP not matching the block mixHash!");
    // BOOST_CHECK_MESSAGE(Ethash::nonce(blockHeaderFromFields) == Ethash::nonce(blockFromRlp),
    // _testname + "nonce in given RLP not matching the block nonce!");

    BOOST_CHECK_MESSAGE( blockHeaderFromFields == blockFromRlp,
        _testname + "However, blockHeaderFromFields != blockFromRlp!" );
    ...
    BOOST_CHECK_MESSAGE( txsFromRlp.size() == txsFromField.size(),
        _testname + "transaction list size does not match" );
    ...
    for ( size_t i = 0; i < txsFromField.size(); ++i ) {
        ...
        BOOST_CHECK_MESSAGE( trField.data() == trRlp.data(),
        ...
        BOOST_CHECK_MESSAGE( trField.nonce() == trRlp.nonce(),
            _testname + "transaction nonce in rlp and in field do not match" );
        BOOST_CHECK_MESSAGE( trField.signature().r == trRlp.signature().r,
            _testname + "transaction r in rlp and in field do not match" );
        BOOST_CHECK_MESSAGE( trField.signature().s == trRlp.signature().s,
            _testname + "transaction s in rlp and in field do not match" );
        BOOST_CHECK_MESSAGE( trField.signature().v == trRlp.signature().v,
            _testname + "transaction v in rlp and in field do not match" );
        BOOST_CHECK_MESSAGE( trField.receiveAddress() == trRlp.receiveAddress(),
            _testname + "transaction receiveAddress in rlp and in field do not match" );
        BOOST_CHECK_MESSAGE( trField.value() == trRlp.value(),
            _testname + "transaction receiveAddress in rlp and in field do not match" );
    ...
    for ( size_t i = 0; i < unclesFromField.size(); ++i )
        BOOST_CHECK_MESSAGE(
            unclesFromField.at( i ).blockHeader() == unclesFromRlp.at( i ).blockHeader(),
            _testname + "block header in rlp and in field do not match at uncles" );
}
